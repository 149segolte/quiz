...
import logo from "../images/Quizzer.png";
	...
	return (
		<div className={styles.verifyBar}>
			<div>
				...
				<strong>{isAuthenticated().user.email}</strong>. A verification
				link has been sent.
			</div>
			<div>
				<button
...
class CreateQuiz extends Component {
	...
	submitQuiz = async (e) => {
		...
		if (!questionsVerified) {
			this.setState({
				error: "Please fill all questions, options and answers correctly",
				loading: false,
			});
			return;
		}
	...
	copyToClipboard = () => {
		...
		textField.innerText = `https://Quizzer.live/${this.state.quizId}`;
	...
	componentDidUpdate(prevProps, prevState) {
		if (prevState.error && prevState.status === "fillingQuiz")
			this.setState({ error: "" });
		if (
			this.state.status === "fillingQuiz" &&
			prevState.questionFilling !== this.state.questionFilling
		) {
			...
			this.setState(
				{ answer, title, optionA, optionB, optionC, optionD },
				() => {
					document.querySelector("select").value = this.state.answer;
				}
			);
		}
	}
	...
	saveAndNext = async () => {
		...
		if (this.state.questionFilling + 1 <= this.state.noOfQuestions) {
			...
			this.setState(
				{ questionFilling: this.state.questionFilling + 1 },
				() => this.form.reset()
			);
		}
	};
	...
	resendVerify = (first) => {
		...
		fetch(`${API_BASE}/sendVerify`, {
			...
			.catch(() =>
				this.setState({ verifyMessage: "Network Error, retry." })
			)
			.finally(() =>
				this.setState({
	...
	componentDidMount() {
		document.title = `Create Quiz | Quizzer`;
	...
	render() {
		if (this.state.checkingVerification)
			return (
				<div>
					<img
		...
		if (!this.state.isVerified)
			return (
				<VerificationBar
					message={this.state.verifyMessage}
					resendVerify={this.resendVerify}
					loading={this.state.sendingVerify}
				/>
			);
		if (this.state.status === "fillingDetails") {
			return (
				<>
					<Helmet>
						<title>Create Quiz on Quizzer</title>
					</Helmet>
					<img
						style={{ marginTop: "90px" }}
						src={logo}
					...
					<div className={styles.details}>
						...
						>
							...
							<div>
								...
								<input
									onChange={this.handleInput("noOfQuestions")}
									required
									id="qnos"
									type="number"
									min="5"
							...
							<div>
								<input
									onChange={this.togglePrivateBoard}
									style={{ width: "auto" }}
									type="checkbox"
									id="private-lead"
								/>
								<label htmlFor="private-lead">
									Private leaderboard{" "}
									<em>
										(leaderboard would be visible to only
										you)
									</em>
								</label>
							</div>
			...
			return (
				<div className={styles.quiz}>
					...
					>
						{Array.from(
							{ length: this.state.noOfQuestions },
							(item, i) => {
								return (
									<button
										onClick={() =>
											this.handleQuestionChange(i + 1)
										}
										key={i}
										style={{
											margin: "10px",
											backgroundColor:
												this.state.verifiedQuestions.includes(
													i + 1
												)
													? "green"
													: "red",
											...
											height: "30px",
											border:
												this.state.questionFilling ===
												i + 1
													? "3px solid rgba(35, 173, 255, 1)"
													: "",
										}}
									>
										{i + 1}
					...
					>
						<div id="questions">
							<Question
								...
								number={this.state.questionFilling}
							/>
							{this.state.error && (
								<p
									style={{
				...
				<>
					...
					<div className={styles.complete}>
						<div style={{ width: "100%" }}>
							...
							<Link
								target="_blank"
								to={`/${this.state.quizId}`}
							>{`https://Quizzer.live/${this.state.quizId}`}</Link>
							<button
							...
							<Link
								target="_blank"
								to={`/${this.state.quizId}/leaderboard`}
							>{`https://Quizzer.live/${this.state.quizId}/leaderboard`}</Link>
						</div>
					</div>
				</>
			);
