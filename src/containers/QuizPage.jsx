...
class QuizPage extends Component {
		...
		fetch(`${API_BASE}/quiz/${this.state.quizId}`)
			...
			.then((data) => {
				if (data.error) {
					...
					document.title = "Quiz Not Found | Quizzer";
				} else {
					const {
						created,
						name,
	...
	handleQuestionChange = (num) => {
		this.setState({
			currrentQuestion: num,
			selectedAnswer: this.state.answers[num - 1],
		});
	...
	saveAnswer = (e) => {
		this.setState({ selectedAnswer: e.target.name }, () => {
			let answers = [...this.state.answers];
			answers[this.state.currrentQuestion - 1] =
		...
		if (this.state.questions[this.state.currrentQuestion]) {
			await this.setState(
				...
				() =>
					this.setState({
						selectedAnswer:
							this.state.answers[this.state.currrentQuestion - 1],
					})
			);
			document.querySelectorAll(".btn-answers").forEach((button) => {
				if (
	...
	render() {
		if (this.state.quiz.name) {
			...
			const { name, duration, created, noOfQuestions, privateBoard } =
				this.state.quiz;
			...
			if (!this.state.starting && !this.state.result) {
				return (
					<>
						<Helmet>
							<title>{name} | Quizzer</title>
							<meta
								name="description"
								content={`Quizzer quiz page for ${name} quiz. Quizzer lets you create multiple-choice question quizzes and share link with friends and family to take + live leaderboard.`}
							/>
						</Helmet>
			...
			if (this.state.starting) {
				return (
					<div className="quiz">
						...
						<div className="activeQuestion bold">
							Question {this.state.currrentQuestion} of{" "}
						...
						{this.state.questions[this.state.currrentQuestion] ? (
							<button
			...
			if (this.state.result) {
				return (
					<>
						<Helmet>
							<title>{name}'s Result | Quizzer</title>
							<meta
								name="description"
								content={`Quizzer quiz page for ${name} quiz. Quizzer lets you create multiple-choice question quizzes and share link with friends and family to take + live leaderboard.`}
							/>
						...
						<div style={{ minHeight: "calc(100vh - 60px)" }}>
							...
							<div className="result-container">
								...
								<div style={{ textAlign: "center" }}>
									...
									<p className="result-percent-intro">
										Your score for '
										<span className="bold">{name}</span>' is{" "}
									</p>
									...
									<p className="result-score-intro">
										Correct Answers / Total Questions
									</p>
									<p className="result-score bold">
										{result.score} / {result.max}
									</p>
									{!privateBoard && (
										<p>
											See where you rank on the{" "}
											<Link
			...
			return (
				<div>
					<img src={logo} alt="Quizzer logo" className="logo-page" />
